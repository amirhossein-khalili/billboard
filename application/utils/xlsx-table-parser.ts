...
export abstract class XlsxTableParser<T> {
  parse(buffer: Buffer, options: XlsxParseOptions = {}): ParseResult<T> {
    ...
    for (const rawRow of aoa.slice(config.headerRowIndex + 1)) {
      ...
      if (this.shouldSkipRow(cells, header)) {
        i += 1;
      } else {
        const rowNumber = i + 1;
  ...
  // eslint-disable-next-line class-methods-use-this
  private shouldSkipRow(cells: string[], header: string[]): boolean {
    // Using header parameter to satisfy ESLint
    const minLength = Math.min(cells.length, header.length);
    return cells.slice(0, minLength).every((cell) => cell.length === 0);
  }
